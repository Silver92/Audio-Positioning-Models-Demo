...
void DBAP::calculate(const std::vector<std::shared_ptr<juce::Point<float>>>& inPos,
                     std::vector<float>& inGainVectors)
{
    ...
    for (int i = 1; i < inPos.size(); i++) {
        ...
        auto length = sqrt(juce::square(x) + juce::square(y) + 1);
    ...
    for (int i = 0; i < ampVectors.size(); i++) {
        sum += 1 / juce::square(ampVectors[i]);
    }
    sum = sqrt(sum);
    ...
    for (int i = 0; i < ampVectors.size(); i++) {
        ...
        juce::Decibels::gainToDecibels(denormalize(1.f / temp)));
        
        std::cout << "DBAP raw gain: " << inGainVectors[i] << std::endl;
        std::cout << "DBAP cal gain: " << juce::Decibels::gainToDecibels(denormalize(1.f / temp)) << std::endl;
    }
