...
void VBAP::calculate(const std::vector<std::shared_ptr<juce::Point<float>>>& inPos,
                     std::vector<float>& inGainVectors)
{
    ...
    for (int i = 0; i < ampVectors.size()-1; i++) {
        if (i == index1) {
            inGainVectors.push_back(juce::Decibels::gainToDecibels(denormalize(gain1)));
        } else if (i == index2) {
            inGainVectors.push_back(juce::Decibels::gainToDecibels(denormalize(gain2)));
        } else {
            inGainVectors.push_back(juce::Decibels::gainToDecibels(0.f));
        }
    }
