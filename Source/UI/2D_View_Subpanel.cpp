...
void TwoDSubpanel::paint(juce::Graphics& g)
{
    
}

void TwoDSubpanel::drawComponents(const std::vector<std::shared_ptr<juce::Point<float>>>& inPos,
                                  ...
                                  const bool hasSoundSource)
{
    ...
    for (int i = 0; i < inPos.size(); i++) {
        ...
        // Draw Speakers
            ...
            std::shared_ptr<Speaker> mTestSpeaker (new Speaker(juce::String(dBFSs[speakerIndex], 1)));
            speakerIndex++;
